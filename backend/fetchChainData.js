...
const processBlockWithExtrinsics = async (api, blockNumber) => {
  try {
    console.log(`Processing block ${blockNumber}`);
    const blockInsertData = [];
    const transactionInsertData = [];
    ...
    for (const [extrinsicIndex, extrinsic] of signedBlock.block.extrinsics.entries()) {
      ...
      const from = isSigned ? signer.toString() : null;
      const methodName = `${section}.${method}`;
      const tip = meta.isSome ? meta.unwrap().tip.toString() : '0';

      // Filter only the events related to this extrinsic
      const extrinsicEvents = allEvents.filter(
        ({ phase }) =>
      ...
      transactionInsertData.push([
        ...
        blockNum,
        from,
        null,              // to_address unknown without specific logic
        '0',               // amount unknown without parsing args
        tip,
        '0',               // gas_fee
        '0',               // gas_value
        methodName,
